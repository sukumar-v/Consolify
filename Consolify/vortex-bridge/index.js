"use strict";
/*
 * Consolify bridge -- a Vortex extension.
 *
 ...
 * so that Consolify can put a mod list on the TV and drive it from a gamepad.
 *
 ...
 * bridge.json beside this file on EVERY request -- Consolify writes a fresh one each time it
 * starts, and re-reading means a Consolify restart never leaves Vortex holding a stale one. The
 ...
 *
 * Copied into %APPDATA%\Vortex\plugins\consolify-bridge by Consolify itself (VortexBackend
...
function log(level, message, meta) {
  try {
    ...
    else console.log("[consolify-bridge]", level, message, meta || "");
...
/** { port, token } or null when Consolify has not written one yet. Read on every request. */
function readConfig() {
  try {
...
async function handle(api, req, res) {
  ...
  if (!cfg) return send(res, 503, { error: "The bridge has no configuration; start Consolify once" });
