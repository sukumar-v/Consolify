...
public sealed class VortexBackend
{
    ...
    public const string PluginFolderName = "consolify-bridge";
    ...
    public async Task<ModManagerStatus> EnsureBridgeAsync(CancellationToken ct)
    {
        ...
        if (await PingAsync(ct) is { } v)
        {
            ...
            if (shipped is not null && _bridgeVersion is not null && _bridgeVersion != shipped)
            {
                ...
                status.Error = $"Vortex is running an older Consolify bridge ({_bridgeVersion}); restart Vortex once to load {shipped}";
                return status;
            }
        ...
        if (IsRunning)
        {
            ...
            status.Running = true;
            status.NeedsRestart = true;
            status.Error = "Vortex is running but has not loaded the Consolify bridge yet. Restart Vortex once";
            return status;
        }
        ...
        status.Error = status.Running
            ? "Vortex started but the Consolify bridge did not answer. Restart Vortex once"
